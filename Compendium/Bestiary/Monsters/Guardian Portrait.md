---
name: "Guardian Portrait"
size: ["Medium"]
type: "Construct"
Alignment: "Unaligned"
AC: 5
HP: 22
speed: ["Walk 0 ft."]
senses: ["Darkvision 60 ft."]
languages: ["Common", "Plus up to two other languages"]
CR: "1"
source: ["Curse of Strahd"]
ruleset: "D&D 5e (2014)"
---

# Guardian Portrait

Medium Construct, Unaligned

**Armor Class**: 5
**Hit Points**: 22
**Speed**: walk 0 ft.
**Senses**: Darkvision 60 ft.
**Languages**: Common, Plus up to two other languages
**Challenge**: 1

| STR | DEX | CON | INT | WIS | CHA |
| --- | --- | --- | --- | --- | --- |
| 1 | 1 | 10 | 14 | 10 | 10 |

### Spellcasting
***Innate Spellcasting.***
The portrait's innate spellcasting ability is Intelligence (spell save 12). The portrait can innately cast the following spells, requiring no material components:
- **3/day each:** [[Counterspell]], [[Crown Of Madness]], [[Hypnotic Pattern]], [[Telekinesis]]

### Traits
***Constructed Nature.*** An animated object doesn't require air, food, drink, or sleep. The magic that animates an object is dispelled when the construct drops to 0 hit points. An animated object reduced to 0 hit points becomes inanimate and is too damaged to be of much use or value to anyone.

***Antimagic Susceptibility.*** The portrait is [[Conditions#incapacitated|Incapacitated]] while in the area of an [[Antimagic Field]]. If targeted by [[Dispel Magic]], the portrait must succeed on a Constitution saving throw against the caster's spell save DC or become [[Conditions#unconscious|Unconscious]] for 1 minute.

***False Appearance.*** While the figure in the portrait remains motionless, the portrait is indistinguishable from a normal painting.