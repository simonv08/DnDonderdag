---
name: "Relentless Juggernaut"
size: ["Large"]
type: "Fiend"
AC: 17
HP: 161
speed: ["Walk 30 ft."]
saving_throws: ["Dexterity", "Wisdom", "Charisma"]
skills: ["Perception +6", "Survival +6"]
senses: ["Darkvision 120 ft."]
languages: ["Understands all languages but can't speak"]
CR: "12"
source: ["Van Richten's Guide to Ravenloft"]
ruleset: "D&D 5e (2014)"
---

# Relentless Juggernaut

Large Fiend, Neutral

**Armor Class**: 17
**Hit Points**: 161
**Speed**: walk 30 ft.
**Saving Throws**: Dexterity, Wisdom, Charisma
**Skills**: Perception +6, Survival +6
**Senses**: Darkvision 120 ft.
**Languages**: Understands all languages but can't speak
**Challenge**: 12

| STR | DEX | CON | INT | WIS | CHA |
| --- | --- | --- | --- | --- | --- |
| 22 | 12 | 22 | 8 | 15 | 16 |

### Traits
***Legendary Resistance (3/Day).*** If the juggernaut fails a saving throw, it can choose to succeed instead.

***Regeneration.*** The juggernaut regains 20 hit points at the start of its turn. If the juggernaut takes radiant damage, this trait doesn't function at the start of its next turn. The juggernaut dies only if it starts its turn with 0 hit points and doesn't regenerate.

***Unusual Nature.*** The juggernaut doesn't require air, food, drink, or sleep.

### Actions
***Multiattack.*** The juggernaut makes two attacks. It can replace one attack with Deadly Shaping if it is ready.

***Executioner's Pick.*** 10 to hit, reach 5 ft., one target. 17 (2d10 + 6) piercing damage, and if the target is a creature, its speed is reduced by 10 feet until the start of the juggernaut's next turn.

***Fist.*** 10 to hit, reach 5 ft., one target. 11 (1d10 + 6) bludgeoning damage, and if the target is a Large or smaller creature, it must succeed on a 18 Strength saving throw or be knocked [[Conditions#prone|Prone]].

***Deadly Shaping (recharge: 5).*** The juggernaut magically shapes a feature of its surroundings into a deadly implement. A creature the juggernaut can see within 60 feet of it must make a 18 Dexterity saving throw. If the saving throw fails, the targeted creature is struck by one of the following (juggernaut's choice):

***Flying Stone.*** The target takes 22 (5d8) bludgeoning damage and is [[Conditions#incapacitated|Incapacitated]] until the start of the juggernaut's next turn, and the implement vanishes.

***Scything Shrapnel.*** The target takes 14 (4d6) slashing damage, and the implement vanishes. At the start of each of its turns, the target takes 10 (3d6) necrotic damage from the wound left by the shrapnel. The wound ends if the target regains any hit points or if a creature uses an action to stanch the wound, which requires a successful 15 Wisdom ([[Skills#Medicine|Medicine]]) check.

### Legendary Actions
***Implacable Advance.*** The juggernaut moves up to its speed, ignoring [[Rules#Difficult Terrain]]. Any object in its path takes 55 (10d10) bludgeoning damage if it isn't being worn or carried.

***Rapid Shaping (Costs 3 Actions).*** The juggernaut recharges Deadly Shaping and uses it.