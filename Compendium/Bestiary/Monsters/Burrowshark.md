---
name: "Burrowshark"
size: ["Medium"]
type: "Humanoid (Human)"
Alignment: "Neutral Evil"
AC: 18
HP: 82
speed: ["Walk 30 ft."]
skills: ["Animal handling +2", "Athletics +6", "Intimidation +3", "Perception +2"]
languages: ["Common"]
CR: "4"
source: ["Princes of the Apocalypse"]
ruleset: "D&D 5e (2014)"
---

# Burrowshark

Medium Humanoid (Human), Neutral Evil

**Armor Class**: 18
**Hit Points**: 82
**Speed**: walk 30 ft.
**Skills**: Animal handling +2, Athletics +6, Intimidation +3, Perception +2
**Languages**: Common
**Challenge**: 4

| STR | DEX | CON | INT | WIS | CHA |
| --- | --- | --- | --- | --- | --- |
| 18 | 12 | 16 | 10 | 11 | 13 |

### Traits
***Bond of the Black Earth.*** The burrowshark is magically bound to a bulette trained to serve as its mount. While mounted on its bulette, the burrowshark shares the bulette's senses and can ride the bulette while it burrows. The bonded bulette obeys the burrowshark's commands. If its mount dies, the burrowshark can train a new bulette to serve as its bonded mount, a process requiring a month.

### Actions
***Multiattack.*** The burrowshark makes three melee attacks.

***Spear.*** 6 to hit, reach 5 ft. or range 20/60 ft., one target. 7 (1d6 + 4) piercing damage, or 8 (1d8 + 4) piercing damage if used with two hands to make a melee attack.

### Reactions
***Unyielding.*** When the burrowshark is subjected to an effect that would move it, knock it [[Conditions#prone|Prone]], or both, it can use its reaction to be neither moved nor knocked [[Conditions#prone|Prone]].