---
name: "Narth Tezrin"
size: ["Medium"]
type: "Humanoid ()"
Alignment: "Chaotic Good"
HP: 18
speed: ["Walk 30 ft."]
skills: ["Insight +4", "Investigation +3", "Perception +6", "Persuasion +5"]
languages: ["Common", "Dwarvish"]
source: ["Storm King's Thunder"]
ruleset: "D&D 5e (2014)"
---

# Narth Tezrin

Medium Humanoid ([object Object]), Chaotic Good

**Armor Class**: undefined
**Hit Points**: 18
**Speed**: walk 30 ft.
**Skills**: Insight +4, Investigation +3, Perception +6, Persuasion +5
**Languages**: Common, Dwarvish
**Challenge**: 

| STR | DEX | CON | INT | WIS | CHA |
| --- | --- | --- | --- | --- | --- |
| 10 | 15 | 10 | 12 | 14 | 16 |

### Traits
***Cunning Action.*** On each of his turns, Narth can use a bonus action to take the [[Actions#Dash|Dash]], [[Actions#Disengage|Disengage]], or [[Actions#Hide|Hide]] action.

***Roleplaying Information.*** Narth sells gear to adventurers, and he also has an adventurous spirit. The Lionshield Coster pays him well, but he longs to make a name for himself. At the same time, he runs a business with his partner Alaestra and knows she wouldn't forgive him if he ran off and never returned. Ideal: "The bigger the risk, the greater the reward." Bond: "I adore my colleague Alestra, and I'd like to do something to impress her." Flaw: "I'll risk life and limb to become a legend.-"

### Actions
***Shortsword.*** 4 to hit, reach 5 ft., one target. 5 (1d6 + 2) piercing damage.

***Hand Crossbow.*** 4 to hit, range 30/120 ft., one target. 5 (1d6 + 2) piercing damage. Narth carries twenty crossbow bolts.