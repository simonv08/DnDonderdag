---
species: "Naga"
size: ["Medium"]
speed: ["Walk 30 ft."]
languages: ["Common", "Other"]
traits: ["[[Naga#Alignment|Alignment]]", "[[Naga#Speed Burst|Speed Burst]]", "[[Naga#Natural Weapons|Natural Weapons]]", "[[Naga#Poison Immunity|Poison Immunity]]", "[[Naga#Poison Affinity|Poison Affinity]]"]
con_mod: 2
int_mod: 1
tool-prof: ["Poisoner's kit"]
source: ["Plane Shift: Amonkhet"]
ruleset: "D&D 5e (2025)"
---

# Naga
## Stats
**Size:** Medium
**Speed:** 30 ft. walking
**Ability Scores:** CON +2, INT +1
**Languages:** Common, Other
**Tool Proficiencies:** Poisoner's kit
## Traits
##### Alignment
Most naga are either neutral or neutral evil in alignment.
##### Speed Burst
By lowering your body to the ground and propelling yourself with your arms, you can move more quickly for a time. As a bonus action on your turn, if you have both hands free, you can increase your walking speed by 5 feet until the end of your turn.
##### Natural Weapons
Your fanged maw and constricting serpentine body are natural weapons, which you can use to make unarmed strikes.

If you hit with your bite, you deal piercing damage equal to 1d4 + your Strength modifier, and your target must make a Constitution saving throw (8 + your proficiency bonus + your Constitution modifier). On a failed save, the target takes 1d4 poison damage.

If you hit with your constrict attack, you deal bludgeoning damage equal to 1d6 + your Strength modifier, and the target is [[Conditions#grappled|Grappled]] (escape 8 + your proficiency bonus + your Strength modifier). Until this grapple ends, the target is [[Conditions#restrained|Restrained]], and you can't constrict another target.
##### Poison Immunity
You are immune to poison damage and can't be [[Conditions#poisoned|Poisoned]].
##### Poison Affinity
You gain proficiency with the [[Poisoner's Kit]].