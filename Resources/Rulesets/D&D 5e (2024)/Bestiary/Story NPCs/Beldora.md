---
name: "Beldora"
size: ["Medium"]
type: "Humanoid ()"
Alignment: "Chaotic Good"
HP: 18
speed: ["Walk 30 ft."]
skills: ["Deception +5", "Insight +3", "Investigation +5", "Perception +5", "Persuasion +5"]
languages: ["Common", "Draconic", "Dwarvish", "Halfling"]
source: ["Storm King's Thunder"]
ruleset: "D&D 5e (2025)"
---

# Beldora

Medium Humanoid ([object Object]), Chaotic Good

**Armor Class**: undefined
**Hit Points**: 18
**Speed**: walk 30 ft.
**Skills**: Deception +5, Insight +3, Investigation +5, Perception +5, Persuasion +5
**Languages**: Common, Draconic, Dwarvish, Halfling
**Challenge**: 

| STR | DEX | CON | INT | WIS | CHA |
| --- | --- | --- | --- | --- | --- |
| 10 | 14 | 10 | 16 | 12 | 16 |

### Traits
***Roleplaying Information.*** Beldora is a member of the harpers who survives using her wits and wiles. She looks like a homeless waif, but she's a survivor who shies away from material wealth. Ideal: "We should all strive to help one another" Bond: "I'll risk my life to protect the powerless." Flaw: "I like lying to people. Makes life more interesting, no?"

### Actions
***Shortsword.*** 4 to hit, reach 5 ft., one target. 5 (1d6 + 2) slashing damage.

***Hand Crossbow.*** 4 to hit, range 30/120 ft., one target. 5 (1d6 + 2) piercing damage. Beldora carries ten crossbow bolts.

### Reactions
***Duck and Cover.*** Beldora adds 2 to her AC against one ranged attack that would hit her. To do so, Beldora must see the attacker and can't be [[Conditions#grappled|Grappled]] or [[Conditions#restrained|Restrained]].