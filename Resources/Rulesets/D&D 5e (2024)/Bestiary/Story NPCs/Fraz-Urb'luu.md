---
name: "Fraz-Urb'luu"
size: ["Large"]
type: "Fiend (Demon)"
Alignment: "Chaotic Evil"
AC: 18
HP: 337
speed: ["Walk 40 ft.", "Fly 40 ft."]
saving_throws: ["Dexterity", "Constitution", "Intelligence", "Wisdom"]
skills: ["Deception +15", "Perception +14", "Stealth +8"]
senses: ["Truesight 120 ft."]
languages: ["All", "Telepathy 120 ft."]
CR: [object Object]
source: ["Monsters of the Multiverse", "Out of the Abyss", "Mordenkainen Presents: Monsters of the Multiverse"]
ruleset: "D&D 5e (2025)"
---

# Fraz-Urb'luu

Large Fiend (Demon), Chaotic Evil

**Armor Class**: 18
**Hit Points**: 337
**Speed**: walk 40 ft., fly 40 ft.
**Saving Throws**: Dexterity, Constitution, Intelligence, Wisdom
**Skills**: Deception +15, Perception +14, Stealth +8
**Senses**: Truesight 120 ft.
**Languages**: All, Telepathy 120 ft.
**Challenge**: [object Object]

| STR | DEX | CON | INT | WIS | CHA |
| --- | --- | --- | --- | --- | --- |
| 29 | 12 | 25 | 26 | 24 | 26 |

### Spellcasting
***Innate Spellcasting.***
Fraz-Urb'luu's spellcasting ability is Charisma (spell save 23). Fraz-Urb'luu can innately cast the following spells, requiring no material components:
- **At will:** [[Alter Self]] (can become Medium when changing his appearance), [[Detect Magic]], [[Dispel Magic]], [[Phantasmal Force]]
- **3/day each:** [[Confusion]], [[Dream]], [[Mislead]], [[Programmed Illusion]], [[Seeming]]
- **1/day each:** [[Mirage Arcane]], [[Modify Memory]], [[Project Image]]

### Traits
***Legendary Resistance (3/Day).*** If Fraz-Urb'luu fails a saving throw, he can choose to succeed instead.

***Magic Resistance.*** Fraz-Urb'luu has advantage on saving throws against spells and other magical effects.

***Magic Weapons.*** Fraz-Urb'luu's weapon attacks are magical.

***Undetectable.*** Fraz-Urb'luu can't be targeted by divination magic, perceived through magical scrying sensors, or detected by abilities that sense demons or fiends.

### Actions
***Multiattack.*** Fraz-Urb'luu makes three attacks: one with his bite and two with his fists.

***Bite.*** 16 to hit, reach 10 ft., one target. 19 (3d6 + 9) piercing damage.

***Fist.*** 16 to hit, reach 10 ft., one target. 22 (3d8 + 9) bludgeoning damage.

### Legendary Actions
***Tail.*** 16 to hit, reach 15 ft., one target. 20 (2d10 + 9) bludgeoning damage. If the target is a Large or smaller creature, it is also [[Conditions#grappled|Grappled]] (escape 24). The [[Conditions#grappled|Grappled]] target is also [[Conditions#restrained|Restrained]]. Fraz-Urb'luu can grapple only one creature with his tail at a time.

***Phantasmal Killer (Costs 2 Actions).*** Fraz-Urb'luu casts [[Phantasmal Killer]], no [[Statuss#concentration|Concentration]] required.