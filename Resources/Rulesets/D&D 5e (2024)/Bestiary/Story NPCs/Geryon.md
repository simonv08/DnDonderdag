---
name: "Geryon"
size: ["Huge"]
type: "Fiend (Devil)"
Alignment: "Lawful Evil"
AC: 19
HP: 300
speed: ["Walk 30 ft.", "Fly 50 ft."]
saving_throws: ["Dexterity", "Constitution", "Wisdom", "Charisma"]
skills: ["Deception +13", "Intimidation +13", "Perception +10"]
senses: ["Truesight 120 ft."]
languages: ["All", "Telepathy 120 ft."]
CR: "22"
source: ["Monsters of the Multiverse", "Mordenkainen Presents: Monsters of the Multiverse"]
ruleset: "D&D 5e (2025)"
---

# Geryon

Huge Fiend (Devil), Lawful Evil

**Armor Class**: 19
**Hit Points**: 300
**Speed**: walk 30 ft., fly 50 ft.
**Saving Throws**: Dexterity, Constitution, Wisdom, Charisma
**Skills**: Deception +13, Intimidation +13, Perception +10
**Senses**: Truesight 120 ft.
**Languages**: All, Telepathy 120 ft.
**Challenge**: 22

| STR | DEX | CON | INT | WIS | CHA |
| --- | --- | --- | --- | --- | --- |
| 29 | 17 | 22 | 19 | 16 | 23 |

### Spellcasting
***Innate Spellcasting.***
Geryon's innate spellcasting ability is Charisma (spell save 21). He can innately cast the following spells, requiring no material components:
- **At will:** [[Alter Self]] (can become Medium when changing his appearance), [[Detect Magic]], [[Geas]], [[Ice Storm]], [[Invisibility]] (self only), [[Locate Object]], [[Suggestion]], [[Wall Of Ice]]
- **1/day each:** [[Divine Word]], [[Symbol]] (pain only)

### Traits
***Legendary Resistance (3/Day).*** If Geryon fails a saving throw, he can choose to succeed instead.

***Magic Resistance.*** Geryon has advantage on saving throws against spells and other magical effects.

***Magic Weapons.*** Geryon's weapon attacks are magical.

***Regeneration.*** Geryon regains 20 hit points at the start of his turn. If he takes radiant damage, this trait doesn't function at the start of his next turn. Geryon dies only if he starts his turn with 0 hit points and doesn't regenerate.

### Actions
***Multiattack.*** Geryon makes two attacks: one with his claws and one with his stinger.

***Claws.*** 16 to hit, reach 15 ft., one target. 23 (4d6 + 9) slashing damage. If the target is Large or smaller, it is [[Conditions#grappled|Grappled]] (24) and is [[Conditions#restrained|Restrained]] until the grapple ends. Geryon can grapple one creature at a time. If the target is already [[Conditions#grappled|Grappled]] by Geryon, the target takes an extra 27 (6d8) slashing damage.

***Stinger.*** 16 to hit, reach 20 ft., one creature. 14 (2d4 + 9) piercing damage, and the target must succeed on a 21 Constitution saving throw or take 13 (2d12) poison damage and become [[Conditions#poisoned|Poisoned]] until it finishes a short or long rest. The target's hit point maximum is reduced by an amount equal to half the poison damage it takes. If its hit point maximum drops to 0, it dies. This reduction lasts until the [[Conditions#poisoned|Poisoned]] condition is removed.

***Teleport.*** Geryon magically teleports, along with any equipment he is wearing and carrying, up to 120 feet to an unoccupied space he can see.

### Legendary Actions
***Infernal Glare.*** Geryon targets one creature he can see within 60 feet of him. If the target can see Geryon, the target must succeed on a 23 Wisdom saving throw or become [[Conditions#frightened|Frightened]] of Geryon until the end of its next turn.

***Swift Sting (Costs 2 Actions).*** Geryon attacks with his stinger.

***Teleport.*** Geryon uses his Teleport action.