---
name: "Aldani (Lobsterfolk)"
size: ["Medium"]
type: "Monstrosity"
Alignment: "Lawful Neutral"
AC: 14
HP: 49
speed: ["Walk 20 ft.", "Swim 30 ft."]
skills: ["Perception +4", "Survival +4"]
senses: ["Darkvision 60 ft."]
languages: ["Common"]
CR: "1"
source: ["Tomb of Annihilation"]
ruleset: "D&D 5e (2025)"
---

# Aldani (Lobsterfolk)

Medium Monstrosity, Lawful Neutral

**Armor Class**: 14
**Hit Points**: 49
**Speed**: walk 20 ft., swim 30 ft.
**Skills**: Perception +4, Survival +4
**Senses**: Darkvision 60 ft.
**Languages**: Common
**Challenge**: 1

| STR | DEX | CON | INT | WIS | CHA |
| --- | --- | --- | --- | --- | --- |
| 13 | 8 | 12 | 10 | 14 | 10 |

### Traits
***Amphibious.*** The aldani can breathe air and water.

### Actions
***Multiattack.*** The aldani makes two attacks with its claws.

***Claw.*** 3 to hit, reach 5 ft., one target. 5 (1d8 + 1) slashing damage, and the target is [[Conditions#grappled|Grappled]] (escape 11). The aldani has two claws, each of which can grapple only one target.