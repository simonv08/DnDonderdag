---
name: "Flameskull"
size: ["Tiny"]
type: "Undead"
Alignment: "Neutral Evil"
HP: 40
speed: ["Walk 0 ft.", "Fly 40 ft."]
skills: ["Arcana +5", "Perception +2"]
senses: ["Darkvision 60 ft."]
languages: ["Common"]
CR: "4"
source: ["Monster Manual", "XMM"]
ruleset: "D&D 5e (2025)"
---

# Flameskull

Tiny Undead, Neutral Evil

**Armor Class**: undefined
**Hit Points**: 40
**Speed**: walk 0 ft., fly 40 ft. (hover)
**Skills**: Arcana +5, Perception +2
**Senses**: Darkvision 60 ft.
**Languages**: Common
**Challenge**: 4

| STR | DEX | CON | INT | WIS | CHA |
| --- | --- | --- | --- | --- | --- |
| 1 | 17 | 14 | 16 | 10 | 11 |

### Spellcasting
***Spellcasting.***
The flameskull is a 5th-level spellcaster. Its spellcasting ability is Intelligence (spell save 13, 5 to hit with spell attacks). It requires no somatic or material components to cast its spells. The flameskull has the following wizard spells prepared:

### Traits
***Illumination.*** The flameskull sheds either dim light in a 15-foot radius, or bright light in a 15-foot radius and dim light for an additional 15 feet. It can switch between the options as an action.

***Magic Resistance.*** The flameskull has advantage on saving throws against spells and other magical effects.

***Rejuvenation.*** If the flameskull is destroyed, it regains all its hit points in 1 hour unless [[Holy Water (flask)]] is sprinkled on its remains or a [[Dispel Magic]] or [[Remove Curse]] spell is cast on them.

### Actions
***Multiattack.*** The flameskull uses Fire Ray twice.

***Fire Ray.*** 5 to hit, range 30 ft., one target. 10 (3d6) fire damage.