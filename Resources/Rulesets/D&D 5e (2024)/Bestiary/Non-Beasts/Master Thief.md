---
name: "Master Thief"
size: ["Medium"]
type: "Humanoid (Any race)"
Alignment: "Any"
AC: 16
HP: 83
speed: ["Walk 30 ft."]
saving_throws: ["Dexterity", "Intelligence"]
skills: ["Acrobatics +7", "Athletics +3", "Perception +3", "Sleight of hand +7", "Stealth +7"]
languages: ["Any one language (usually Common) plus Thieves' cant"]
CR: "5"
source: ["Volo's Guide to Monsters", "Infernal Machine Rebuild", "Mordenkainen Presents: Monsters of the Multiverse"]
ruleset: "D&D 5e (2025)"
---

# Master Thief

Medium Humanoid (Any race), A

**Armor Class**: 16
**Hit Points**: 83
**Speed**: walk 30 ft.
**Saving Throws**: Dexterity, Intelligence
**Skills**: Acrobatics +7, Athletics +3, Perception +3, Sleight of hand +7, Stealth +7
**Languages**: Any one language (usually Common) plus Thieves' cant
**Challenge**: 5

| STR | DEX | CON | INT | WIS | CHA |
| --- | --- | --- | --- | --- | --- |
| 11 | 18 | 14 | 11 | 11 | 12 |

### Traits
***Cunning Action.*** On each of its turns, the thief can use a bonus action to take the [[Actions#Dash|Dash]], [[Actions#Disengage|Disengage]], or [[Actions#Hide|Hide]] action.

***Evasion.*** If the thief is subjected to an effect that allows it to make a Dexterity saving throw to take only half damage, the thief instead takes no damage if it succeeds on the saving throw, and only half damage if it fails.

***Sneak Attack (1/Turn).*** The thief deals an extra 14 (4d6) damage when it hits a target with a weapon attack and has advantage on the attack roll, or when the target is within 5 feet of an ally of the thief that isn't [[Conditions#incapacitated|Incapacitated]] and the thief doesn't have disadvantage on the attack roll.

### Actions
***Multiattack.*** The thief makes three attacks with its shortsword.

***Shortsword.*** 7 to hit, reach 5 ft., one target. 7 (1d6 + 4) piercing damage.

***Light Crossbow.*** 7 to hit, range 80/320 ft., one target. 8 (1d8 + 4) piercing damage.

### Reactions
***Uncanny Dodge.*** The thief halves the damage that it takes from an attack that hits it. The thief must be able to see the attacker.